...
class PatchEmbedBlur(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, filter_size=5, learnable=True):
        super().__init__()
        ...
        self.proj = nn.Sequential(
                BlurPool(channels=in_chans, filt_size=filter_size, stride=patch_size[0], learnable=learnable),
                nn.Conv2d(in_chans, embed_dim, kernel_size=1, stride=1, bias=True),
...
class LipRegSwinTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 wavelet_level=2, wavelet_method="haar", jacobian_delta=0.5, k=1.0, 
                 filter_size=5, learnable=True,
                 **kwargs):
        super().__init__()
        ...
        self.patch_embed = PatchEmbedBlur(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None, 
            filter_size=self.filter_size, learnable=self.learnable)
