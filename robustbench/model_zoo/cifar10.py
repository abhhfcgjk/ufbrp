...
from robustbench.model_zoo.architectures.lipreg_aa import LipReg, LipReg_aa, ResNet, ResNetBlur, BottleneckBlur
...
class WideResNet_base(WideResNetBase):
    def __init__(self, num_classes=10, depth=94, widen_factor=16):
...
class ResNet_v2_base(ResNet_v2):
    def __init__(self, block=Bottleneck_v2, num_blocks=[3, 4, 6, 3], num_classes=10):
        super().__init__(block, num_blocks, num_classes)
        # self.mu = torch.tensor([0.4914, 0.4822, 0.4465]).view(1, 3, 1, 1)
        # self.sigma = torch.tensor([0.2023, 0.1994, 0.2010]).view(1, 3, 1, 1)
        self.normalize = ImageNormalizer((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
...
class ResNet_Blur(ResNetBlur):
    def __init__(self, block=BottleneckBlur, num_blocks=[3, 4, 6, 3], num_classes=10, filter_size=5, learnable=True):
